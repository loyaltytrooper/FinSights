...
func ParseAxisPDF(fileName string, password string) (models.Transactions, map[string][]string) {
	...
	for _, f := range files {
		if !f.IsDir() {
			ReadAxisStatement("act_"+fileName[0:len(fileName)-4]+"/"+f.Name(), &transactions, &csvData)
		}
	}
	...
	for scanner.Scan() {
		if scanner.Text()[len(scanner.Text())-1] == 'j' {
			if len(scanner.Text()) > 5 {
				word := strings.TrimSpace(scanner.Text()[1 : len(scanner.Text())-3])
				if strings.Compare(word, "OPENING BALANCE") == 0 {
					fmt.Println("Found")
					for scanner.Scan() {
						if scanner.Text()[len(scanner.Text())-1] != 'j' {
							...
							tempBalance, err := strconv.ParseFloat(strings.TrimSpace(scanner.Text()[1:len(scanner.Text())-3]), 64)
							if err == nil {
								prevBalance = tempBalance
								fmt.Println("Prev Balance")
				...
				if strings.Compare(word, "TRANSACTION TOTAL") == 0 && foundTable == true {
					foundTable = false
